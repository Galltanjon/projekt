...
def display_best_match(best_match, needed_ingredients, extra_ingredients):
    """Displays information about the best match, needed ingredients, and extra ingredients.
    If there is a best match, extra_ingredient_cost is returned, otherwise 0 (None) is returned"""
    if best_match:
...
def main(menus, options):
    """Initiates menu read and options from the selected bar.
    ...
    #Displays information about the best match, needed ingredients, and extra ingredients
    extra_ingredient_cost = display_best_match(best_match, needed_ingredients, extra_ingredients)
    total_order_cost += extra_ingredient_cost

    while True:
        accept_order = input("'yes' to complete the main order, 'no' to add addons, 'q' to quit\n>")
        if accept_order == "yes":
            total_order_cost += best_match.price
            break  
        elif accept_order == "no":
            total_order_cost, addons_wishes = addons(options, user_wishes_list, total_order_cost)
            total_order_cost += best_match.price
            break  
        elif accept_order == "q":
            sys.exit()
    ...
    receipt(best_match, addons_wishes, extra_ingredients, total_order_cost)
